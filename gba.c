...
void waitForVBlank(void) { // TODO: IMPLEMENT
  // (1) Write a while loop that loops until we're NOT in vBlank anymore:
  // (This prevents counting one VBlank more than once if your app is too fast)
  while (SCANLINECOUNTER > 160)
    ;
  // (2) Write a while loop that keeps going until we're in vBlank:
  while (SCANLINECOUNTER < 160)
    ;
...
void setPixel(int row, int col, u16 color) { // TODO: IMPLEMENT
  videoBuffer[OFFSET(row, col, WIDTH)] = color;
}

void drawRectDMA(int row, int col, int width, int height, volatile u16 color) { // TODO: IMPLEMENT
...
void drawFullScreenImageDMA(const u16 *image) { // TODO: IMPLEMENT
  DMA[3].dst = &videoBuffer[OFFSET(0, 0, WIDTH)];
  DMA[3].src = &image[OFFSET(0, 0, WIDTH)];
  DMA[3].cnt = (240 * 160) | DMA_ON | DMA_DESTINATION_INCREMENT;
}
...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) { // TODO: IMPLEMENT
  for (int i = 0; i < height; i++) {
    ...
    DMA[3].cnt = (width) | DMA_ON | DMA_DESTINATION_INCREMENT; // DMAsourceinc?
  }
}

// Fills the entire GBA screen with a specified color
void fillScreenDMA(volatile u16 color) { // TODO: IMPLEMENT
